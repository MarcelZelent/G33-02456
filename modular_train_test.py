...
def training(model=SpectrVelCNNRegr):
    ...
    optimizer = OPTIMIZER(model.parameters(), lr=LEARNING_RATE, weight_decay=1e-5)
    ...
    for epoch in range(EPOCHS):
        ...
        # Reset test loss
        running_test_loss = torch.tensor(0.0, device=DEVICE)
        ...
        toc = time.time()
        print(f"Test loss calculated in {toc-tic:.2f} seconds")
